...
pub async fn get_db_connection_pool() -> &'static DatabaseConnection {
    DB_CONNECTION.get_or_init(|| async {
        let db_uri = std::env::var("DATABASE_URL").unwrap_or("postgres://demo-user:demo-password@localhost:5432/demo".to_owned());
        ...
        Database::connect(opt).await.expect("connect database error")
    }).await
}
